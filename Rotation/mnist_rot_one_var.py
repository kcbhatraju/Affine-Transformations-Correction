...
# ONEVAR MEANS EACH BATCH IN THE FAKE DISTRIBUTION SHARES ROTATION VARIABLES
...
class Generator(nn.Module):
    def __init__(self):
        super().__init__()
        self.rot = (3*PI/2).clone().requires_grad_()
    
    def forward(self,idx):
        for _, (batch, _) in enumerate(fake_loader,idx):
            rot = torch.stack([
                torch.stack([torch.cos(self.rot), -torch.sin(self.rot), torch.zeros(())]),
                torch.stack([torch.sin(self.rot), torch.cos(self.rot), torch.zeros(())])
                ])
            rot_mat = rot[None, ...].repeat(batch.shape[0],1,1)
            grid = F.affine_grid(rot_mat, batch.size())
            full = F.grid_sample(batch, grid)
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (real, _) in enumerate(loader):
        ...
        with torch.no_grad():
            progress(batch_idx+1,len(loader),gen_loss=round(float(lossG.mean().numpy()),2),discrim_loss=round(float(lossD.mean().numpy()),2),rot=round(float(gen.rot.numpy()),2))
