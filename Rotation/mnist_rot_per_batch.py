...
class Generator(nn.Module):
    def __init__(self):
        super().__init__()
        self.rot = torch.full((batch_size,),(PI)).requires_grad_()
    ...
    def forward(self,idx):
        for _, (batch, _) in enumerate(fake_loader,idx):
            ...
            for i, img in enumerate(batch):
                ...
                rot = torch.stack([
                    torch.stack([torch.cos(self.rot[i]), -torch.sin(self.rot[i]), torch.zeros(())]),
                    torch.stack([torch.sin(self.rot[i]), torch.cos(self.rot[i]), torch.zeros(())])
                    ])
