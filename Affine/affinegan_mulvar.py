...
class Generator(nn.Module):
    def __init__(self):
        super().__init__()
        self.rot = torch.full((batch_size,),PI).float().requires_grad_()
        self.trans = torch.full((batch_size,2),0.).float().requires_grad_()
        self.scale = torch.full((batch_size,),1.).float().requires_grad_()
        self.shear = torch.full((batch_size,2),0.).float().requires_grad_()
