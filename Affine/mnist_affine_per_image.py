...
# All images in the dataset have different affine transformation variables
...
class Generator(nn.Module):
    def __init__(self):
        super().__init__()
        self.rot = torch.full((len(fake_dataset),),(0.)).requires_grad_()
        self.trans = torch.full((len(fake_dataset),2),(0.)).requires_grad_()
        self.scale = torch.full((len(fake_dataset),),(1.)).requires_grad_()
